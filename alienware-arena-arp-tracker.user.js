...
(function () {
    ...
    const I18N = {
        ...
        zh: {
            ...
            infoDescriptionText: '显示什么会过期、什么时候过期：在站时间和 Twitch 于 UTC 00:00 重置，每日任务是一次性的、不会回来，Steam 任务从周一到周一，活动日历可能还有一天等你领取。当同时有两个 Steam 任务、而固定那个的游戏又出现在可自选任务的列表里时，选它就能让同一个小时对两个都算数。如果那是一款你还没有的免费游戏，把它加进 Steam 库可能就足以让它出现在那个列表里。Discord 只在周一至周五发放，周末那一行会安静下来而不是催你，点击它会打开「Arena Connect」频道，也就是投票的地方。战斗通行证的代币在赛季结束时会被清空，所以面板会告诉你它们现在值多少 ARP。每一行的提示都会说明它归哪个时钟管，另外两个倒计时——当天和 Steam 周期——会自动刷新。在赠品页面上，它会读取该赠品按国家和等级的库存，在你点击任何按钮之前就告诉你是否有属于你的密钥。在 Marketplace 和宝库中，它会标注每张卡片：买得起、需要更高等级，或者已售完。它还能提醒你三次——当天结束前、Steam 周期结束前六小时，以及新的一天开始时——用一个必须关闭的对话框。没有声音：在这个站点上试了三种办法，浏览器每次都拦截，而同样的代码在别的站点却能响；从页面里已经没有可修的了。对话框、标签页上的 👽 标记和面板上的横幅都能传到你这里，而且都不需要我们拿不到的许可。它每 15 分钟会自行重新读取一次，⟳ 按钮则立刻重读全部数据。它只读取：不领取、不出价、不参与任何活动，因为这些都要过验证码。',
